...
define(['lodash', 'core/promise', 'core/dataProvider/proxy'], function(_, Promise, proxyFactory) {
    ...
    QUnit.test('module', function(assert) {
        ...
        assert.notStrictEqual(
            proxyFactory(),
            proxyFactory(),
            'The proxyFactory factory provides a different object on each call'
        );
    });
            ...
            _.defaults(
                {
                    init: function(config) {
                        ...
                        assert.deepEqual(
                            config,
                            initConfig,
                            'The proxyFactory has provided the config object as a parameter'
                        );
    ...
    QUnit.test('proxy.destroy()', function(assert) {
        ...
        proxy = proxyFactory('default').on('destroy', function(promise) {
            assert.ok(true, 'The proxyFactory has fired the "destroy" event');
            assert.ok(
                promise instanceof Promise,
                'The proxyFactory has provided the promise through the "destroy" event'
            );
        });
    ...
    QUnit.test('proxy.create()', function(assert) {
            ...
            _.defaults(
                {
                    create: function(params) {
                        ...
                        assert.deepEqual(
                            params,
                            expectedParams,
                            'The delegated method received the expected parameters'
                        );
                        return Promise.resolve();
                    }
                },
        ...
        proxy = proxyFactory('default').on('create', function(promise, params) {
    ...
    QUnit.test('proxy.read()', function(assert) {
        ...
        proxy = proxyFactory('default').on('read', function(promise, params) {
    ...
    QUnit.test('proxy.write()', function(assert) {
        ...
        proxy = proxyFactory('default').on('write', function(promise, params) {
    ...
    QUnit.test('proxy.remove()', function(assert) {
        ...
        proxy = proxyFactory('default').on('remove', function(promise, params) {
    ...
    QUnit.test('proxy.action()', function(assert) {
        ...
        proxy = proxyFactory('default').on('action', function(promise, action, params) {
        ...
        proxy
            .action(expectedAction, expectedParams)
            .then(function() {
                assert.ok(false, 'The proxy must be initialized');
            })
            .catch(function() {
                ...
                proxy
                    .on('read', function(promise, params) {
                        ...
                        if (extraParamsSet) {
                            assert.deepEqual(
                                params,
                                _.merge({}, expectedParams, extraParams),
                                'The proxyFactory has provided the params through the "read" event with extra parameters'
                            );
                            ...
                            assert.deepEqual(
                                params,
                                expectedParams,
                                'The proxyFactory has provided the params through the "read" event without extra parameters'
                            );
                    ...
                    .on('write', function(promise, params) {
                        ...
                        if (extraParamsSet) {
                            assert.deepEqual(
                                params,
                                _.merge({}, expectedParams, extraParams),
                                'The proxyFactory has provided the params through the "write" event with extra parameters'
                            );
                            ...
                            assert.deepEqual(
                                params,
                                expectedParams,
                                'The proxyFactory has provided the params through the "write" event without extra parameters'
                            );
                    ...
                    .on('action', function(promise, action, params) {
                        ...
                        assert.equal(
                            action,
                            expectedAction,
                            'The proxyFactory has provided the action through the "action" event'
                        );
                        ...
                        if (extraParamsSet) {
                            assert.deepEqual(
                                params,
                                _.merge({}, expectedParams, extraParams),
                                'The proxyFactory has provided the params through the "action" event with extra parameters'
                            );
                            ...
                            assert.deepEqual(
                                params,
                                expectedParams,
                                'The proxyFactory has provided the params through the "action" event without extra parameters'
                            );
    ...
    QUnit.test('proxy with middleware', function(assert) {
        ...
        proxy
            ...
            .then(function() {
                assert.equal(
                    proxy.getMiddlewares(),
                    middleware,
                    'The proxy should return the registered middleware handler'
                );
    ...
    QUnit.test('proxy #error', function(assert) {
        ...
        proxy = proxyFactory('default').on('error', function(err) {
