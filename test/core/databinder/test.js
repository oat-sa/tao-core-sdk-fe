...
define(['jquery', 'core/databinder'], function($, DataBinder) {
    ...
    QUnit.test('Array assignment', function(assert) {
        ...
        assert.strictEqual(
            $sectionParts.find('li').length,
            model.testParts[0].assessmentSections[0].sectionParts.length,
            'the same number of nodes has been inserted'
        );
        assert.equal(
            $sectionParts.find('li:first').text(),
            model.testParts[0].assessmentSections[0].sectionParts[0].href,
            'the first item contains the value'
        );
        assert.equal(
            $sectionParts.find('li:last').text(),
            model.testParts[0].assessmentSections[0].sectionParts[3].href,
            'the last item contains the value'
        );
    });
    ...
    QUnit.test('Array value re-order', function(assert) {
        ...
        assert.equal(
            $firstPart.find('[data-bind="href"]').text(),
            model.testParts[0].assessmentSections[0].sectionParts[0].href,
            'the first item contains the value'
        );
        assert.equal(
            $thirdPart.find('[data-bind="href"]').text(),
            model.testParts[0].assessmentSections[0].sectionParts[2].href,
            'the third item contains the value'
        );
        ...
        assert.equal(
            model.testParts[0].assessmentSections[0].sectionParts[1].href,
            'http://tao26.localdomain/bertao.rdf#i138356893768139',
            'the model value order has been updated'
        );
        assert.equal(
            model.testParts[0].assessmentSections[0].sectionParts[2].href,
            'http://tao26.localdomain/bertao.rdf#i138356893796686',
            'the model value order has been updated'
        );
        ...
        assert.equal(
            model.testParts[0].assessmentSections[0].sectionParts[1].index,
            1,
            'the model value index has been updated'
        );
    });
    ...
    QUnit.test('Array value remove', function(assert) {
        ...
        assert.strictEqual(
            model.testParts[0].assessmentSections[0].sectionParts[0].index,
            0,
            'model element has been removed'
        );
        assert.strictEqual(
            $sectionParts.find('li:nth-child(1)').data('bind-index'),
            '0',
            'the node index is up to date'
        );
        assert.equal(
            $sectionParts.find('li:nth-child(1) [data-bind="href"]').text(),
            'http://tao26.localdomain/bertao.rdf#i138356893796686',
            'the model value order has been updated'
        );
        ...
        $sectionParts
            .find('li:nth-child(1)  [data-bind="href"]')
            .text('http://new.url')
            .trigger('change');
    ...
    QUnit.test('Array value add', function(assert) {
        ...
        $newSection = $(
            "<li><span data-bind='identifier'>sectionpart-5</span><span data-bind='href'>http://new.rdf#test</span></li>"
        );
        ...
        assert.equal(
            model.testParts[0].assessmentSections[0].sectionParts[4].identifier,
            'sectionpart-5',
            'model element has been added'
        );
        assert.equal(
            model.testParts[0].assessmentSections[0].sectionParts[4].href,
            'http://new.rdf#test',
            'model element has been added'
        );
    });
    ...
    QUnit.test('Add value which is not represented in the model', function(assert) {
        ...
        assert.equal(
            model.testParts[0].assessmentSections[0].sectionParts[3].visible,
            true,
    ...
    QUnit.test('Rm binding', function(assert) {
        ...
        assert.equal(
            model.testParts[0].assessmentSections[0].sectionParts.length,
            4,
            'The section parts contains 4 elements'
        );
        $container.find('input:last-child').trigger('change');
        assert.equal(
            model.testParts[0].assessmentSections[0].sectionParts.length,
            3,
            'A section parts has been remvoved'
        );
    });
    ...
    QUnit.test('Deleter binding', function(assert) {
        ...
        assert.equal(
            model.testParts[0].assessmentSections[0].sectionParts.length,
            4,
            'The model contains the 4 section parts'
        );
        ...
        $container.on('delete.deleter', function() {
            $container.on('delete.binder', function(e, newModel) {
                assert.equal(
                    newModel.testParts[0].assessmentSections[0].sectionParts.length,
                    3,
                    'The model contains now one less section part'
                );
            });
