...
define(['core/router', 'context'], function(router, context) {
    ...
    QUnit.cases
        .init([
            ...
            },
            {
                ...
                url:
                    '/tao/Main/index?structure=delivery&ext=taoDeliveryRdf&section=manage_delivery_assembly&uri=http%3A%2F%2Ftaoce.taocloud.org%2Ftao.rdf%23i1538125384200365',
                route: {
                    extension: 'tao',
                    module: 'Main',
                    action: 'index',
    ...
    QUnit.test('dispatch', function(assert) {
        ...
        router
            .dispatch(['/tao/Main/test', '/taoItems/Item/test'])
            .then(function() {
                //The global value is set by the controller start
                assert.deepEqual(
                    window.controllerStarted,
                    {
                        tao: true,
                        taoItems: true
                    },
