define(['jquery', 'util/typeCaster'], function($, typeCaster) {
    ...
    var strToBoolStringData, strToBoolBooleanData, strToBoolOtherData;
    ...
    QUnit.cases.init(strToBoolStringData).test('strToBool() with String input', function(data, assert) {
        assert.equal(typeCaster.strToBool(data.value), data.result, 'correct value returned with no default value');
        assert.equal(
            typeCaster.strToBool(data.value, true),
            data.result,
            'correct value returned with default value === true'
        );
        assert.equal(
            typeCaster.strToBool(data.value, false),
            data.result,
            'correct value returned with default value === false'
        );
    });

    strToBoolBooleanData = [{ value: true, title: 'true => true' }, { value: false, title: 'false => false' }];

    QUnit.cases.init(strToBoolBooleanData).test('strToBool() with Boolean input', function(data, assert) {
    ...
    QUnit.cases.init(strToBoolOtherData).test('strToBool() with other type of data', function(data, assert) {
        assert.equal(typeCaster.strToBool(data.value), false, 'non string value with no defaultValue returns false');
        assert.equal(
            typeCaster.strToBool(data.value, true),
            true,
            'non string value with defaultValue === true returns true'
        );
        assert.equal(
            typeCaster.strToBool(data.value, false),
            false,
            'non string value with no defaultValue === false returns false'
        );
    });
});
