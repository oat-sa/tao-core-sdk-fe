...
UrlParser.prototype.getUrl = function (exclude) {
    ...
    if (this.data) {
        if (this.data.hostname && !_.includes(exclude, 'host')) {
            ...
            //the value of the port seems to be different regarding the browser, so we prevent adding port if not usual
...
/**
 * is the current URL in the same domain as the one in parameter
